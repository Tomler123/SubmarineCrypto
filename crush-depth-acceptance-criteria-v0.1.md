...
- **EN-8** Rejection reasons are reported in a fixed precedence: loss-lock → entry-closed → position-open → balance → no-price. The principle is that a **condition the player cannot clear is never masked by one that clears on its own** — the ranks run from session-terminal (loss-lock) through round-terminal (`ENTRY_CLOSED`, which nothing reopens this round) to round-scoped-but-self-clearing (`POSITION_OPEN`, which resolves when the position settles), then to the actionable and the systemic. Specifically, a loss-locked player (RP-2) — specifically, a loss-locked player (RP-2) MUST NOT be told "insufficient balance", which implies that depositing more would let them continue. Test: hold every condition from a given rank downward simultaneously and assert the higher-ranked code is returned. Every rejection leaves the wallet untouched regardless of rank.
...
- **CR-6b** τ-alignment of auto-order triggers: a take-profit, stop-loss or max-win trigger evaluated at tick *n* MUST be evaluated against `M_t` at the **same τ** the crush line at tick *n* is tested against and the multiplier displayed at tick *n* is drawn from. CR-1 fixes the per-tick order as crush → auto-order triggers → ascent settlement; the single τ advance at the top of the tick is what makes all three see one τ. A trigger evaluated against a stale multiplier (τ=n−1 while the crush line is at τ=n) is the same defect class as the CR-6 crush-line drift and carries the same severity: **a release blocker**, because oxygen makes `M` strictly time-dependent, so one stale tick is one tick of edge applied to the wrong side of a payout decision. Test: across a full round with a TP armed just above the reachable band, assert for every tick with a position open that the multiplier the trigger tested equals `positionMultiplier` at that tick's τ, and that no trigger fires on a τ the displayed multiplier never took. A trigger firing at tick *n* starts an ascent at `t_r = t(n)` (AO-2) and therefore settles at a later tick's `M` — CR-6b constrains the τ of the **trigger decision**, not of the settlement.
...
- **AO-5** Max-win: `M_t ≥ 50` force-triggers an ascent identical to AO-2; final payout is capped at min(50 × stake at settlement M, $10,000). The cap and the trigger are **separate mechanisms and land separately**: the cap is the unconditional PL-4 clamp on every settlement, the trigger is the auto-surface that stops a position running further once the cap can no longer pay more. A build with the trigger but no clamp overpays on a gap; a build with the clamp but no trigger pays correctly but lets a capped position keep risking a crush for no upside.
